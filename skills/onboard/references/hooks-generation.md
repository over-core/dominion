...
```bash
#!/bin/sh
# Generated by Dominion v0.5.0 — do not edit manually
...
AUTO="false"
if [ -f ".dominion/config.toml" ]; then
  AUTO=$(grep '^auto_continue = ' .dominion/config.toml 2>/dev/null | head -1 | cut -d' ' -f3 | tr -d '"' || echo "false")
fi
...
if [ "$STEP" != "idle" ]; then
  ...
  if [ "$AUTO" = "true" ] && [ "$CB_STATE" = "closed" ] && [ "$STATUS" = "active" ]; then
    echo ""
    echo ">>> PIPELINE READY: Step '${STEP}' is active. Run /dominion:orchestrate --resume to continue the pipeline. <<<"
  fi

  if [ "$CB_STATE" = "open" ]; then
    echo ""
    echo ">>> CIRCUIT BREAKER OPEN: Pipeline halted. Fix issues and run /dominion:orchestrate to review. <<<"
  fi
  echo "---"
fi
exit 0
