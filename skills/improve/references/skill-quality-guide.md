...
- Improve references go in `skills/improve/references/` — for agent/skill authoring guides
...
| "The system should validate inputs" | "Call `mcp__dominion__phase_status()`. If errors: list them and STOP." |

**Rules:**
- Specify tools by name: "Read file X" not "check file X." "Call `mcp__dominion__*`" not "update the state."
...
Pipeline skills use MCP tools for state transitions — the pipeline_next tool handles step advancement automatically. Standalone skills skip state management.
...
- [ ] `name` is kebab-case and unique across existing skills (check `skills/*/SKILL.md`)
...
- [ ] Tool usage is specific — named tools ("Read", "Call `mcp__dominion__*`"), not vague ("analyze", "check")
...
- [ ] State management via `mcp__dominion__*` tools — not direct TOML edits
