...
- Search existing agent templates (`.dominion/agents/*/agent.toml`) for methods in related domains
...
|----------|------|---------------|
| 1 | dominion MCP tools | **Mandatory for every agent.** All `.dominion/` data via `mcp__dominion__*` tools — plan, progress, knowledge, signals, memory. |
...
1. Read `.dominion/agents/*/agent.toml` — list all existing agents' names and purposes
